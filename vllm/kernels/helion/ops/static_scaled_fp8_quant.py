...
def generate_inputs() -> dict[str, tuple[Any, ...]]:
    ...
    group_shape_list = [(-1, -1), (-1, 1), (1, -1), (1, 128), (128, 128)]
    ...
    for hidden_size, group_shape, num_tokens in product(
        ...
        if num_tokens % group_m != 0 or hidden_size % group_n != 0:
            continue
...
def static_scaled_fp8_quant(
    ...
    m_block_size = hl.register_block_size(1, helion.next_power_of_2(num_tokens))
    gm_block_size = 1 if num_groups_m == 1 else helion.cdiv(m_block_size, group_m)
    m_per_group_block_size = (
        1 if num_groups_m == num_tokens else min(m_block_size, group_m)
    )

    n_block_size = hl.register_block_size(1, helion.next_power_of_2(hidden_size))
    gn_block_size = helion.cdiv(n_block_size, group_n)
    n_per_group_block_size = min(n_block_size, group_n)

    for tile_gm, tile_m, tile_gn, tile_n in hl.tile(
        [num_groups_m, group_m, num_groups_n, group_n],
        block_size=[
            gm_block_size,
            m_per_group_block_size,
            gn_block_size,
            n_per_group_block_size,
        ],
    ):
        ...
        # offset inside group
        m_offset = tile_m.begin + hl.arange(tile_m.block_size)
        n_offset = tile_n.begin + hl.arange(tile_n.block_size)
        ...
        group_mask = (gm_idx[:, None] < num_groups_m) & (gn_idx[None, :] < num_groups_n)
        elem_mask = (
            (gm_idx[:, None, None, None] < num_groups_m)
            & (m_offset[None, :, None, None] < group_m)
            & (gn_idx[None, None, :, None] < num_groups_n)
            & (n_offset[None, None, None, :] < group_n)
        )
        ...
        # input tile shape:  [tile_gm, tile_m, tile_gn, tile_n]
        x_blk = hl.load(
            input,
            [m_blk, n_blk],
        ...
        # scale tile shape:  [tile_gm, 1, tile_gn, 1]

...
def static_scaled_fp8_quant_dispatch(
    ...
    static_scaled_fp8_quant(output, input, scale_2d, group_m, group_n)


@static_scaled_fp8_quant.register_input_generator  # type: ignore[misc]
def generate_inputs() -> dict[str, tuple[Any, ...]]:
    ...
    for num_tokens, hidden_size, group_shape in product(
        num_tokens_list, hidden_size_list, group_shape_list
    ):
        ...
        inputs[config_key] = (output, input, scale_2d, group_m, group_n)

    return inputs


@static_scaled_fp8_quant.register_config_picker  # type: ignore[misc]
def pick_config(args: tuple[Any, ...], config_keys: list[str]) -> str | None:
    ...
    output, input, scale_2d, group_m, group_n = args
    num_tokens, hidden_size = input.shape
...
def static_scaled_fp8_quant_baseline(
    output: torch.Tensor,
    input: torch.Tensor,
    scale: torch.Tensor,
    group_m: int,
    ...
    torch.ops._C.static_scaled_fp8_quant(
        output, input, scale, (group_shape_m, group_shape_n)
    )

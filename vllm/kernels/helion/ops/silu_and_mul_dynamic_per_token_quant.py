...
def silu_and_mul_dynamic_per_token_quant(
    ...
    for tile_m in hl.tile(num_tokens, block_size=1):
        ...
        for tile_n in hl.tile(intermediate_size):
            ...
            x_b_blk = hl.load(input, [tile_m, tile_n.index + intermediate_size], extra_mask=((tile_n.index + intermediate_size) < two_intermediate_size)[None, :]).to(torch.float32)
...
def baseline(
    ...
    out, scale_out = compiled_layer(result, input, scale, scale_ub)

...
def per_token_group_fp8_quant(
    ...
    for tile_m in hl.tile(num_tokens, block_size=1):
        ...
        for tile_gn in hl.tile(groups_per_row):
            ...
            hl.store(output_q, [m_blk, n_blk], y_q_blk, extra_mask=mask)


@per_token_group_fp8_quant.register_input_generator  # type: ignore[misc]
...
@per_token_group_fp8_quant.register_config_picker  # type: ignore[misc]
...
def per_token_group_fp8_quant_baseline(
    input: torch.Tensor,  # [num_tokens, hidden_size]
    output_q: torch.Tensor,  # [num_tokens, hidden_size]
    output_s: torch.Tensor,  # [num_tokens, groups_per_row]
    group_size: int,

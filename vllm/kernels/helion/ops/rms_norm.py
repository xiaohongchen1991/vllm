...
def rms_norm(
    ...
    assert input.dtype == result.dtype
    ...
    if residual is not None:
        ...
        r = residual.view(-1, input.shape[-1])

    x = input.flatten(0, -2)
    assert result.is_contiguous()
    y = result.view(-1, input.shape[-1])
...
def rms_norm_baseline(
    ...
    if residual is not None:
        result.copy_(input)
        torch.ops._C.fused_add_rms_norm(result, residual, weight, epsilon)
    else:
